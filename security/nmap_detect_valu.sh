...
# Define scan_host function to scan a single host for vulnerabilities
scan_host() {
  # Check if the host is up
  nmap -sn $1 > /dev/null
  # If the host is up, scan it with NSE scripts
  if [ $? -eq 0 ]; then
    echo "Scanning host $1 for vulnerabilities"
    # Use the -sV flag to enable version detection and the --script flag to run NSE scripts
    # You can specify which scripts or categories of scripts to run, such as default, safe, vuln, etc.
    # For example, the following command runs the default and safe scripts, plus the vuln category
    nmap -sV --script "default,safe,vuln" $1 
  else
    echo "Host $1 is down"
  fi
}

# Define scan_range function to scan a range of hosts for vulnerabilities
scan_range() {
...
file_name="vuln_scan_results_$datetime.txt"
...
mail -s "Vulnerability scan completed on $datetime" -a $file_name user@example.com < /dev/null
