...
check_ssh_logs() {
  for file in /var/log/secure*; do
    if file $file | grep -q gzip; then
      zcat -q $file | awk -v start_date="$start_date" -v end_date="$end_date" -v start_time="$start_time" -v end_time="$end_time" -v username="$username" -v ip_address="$ip_address" '
        BEGIN {
          split(start_date, sd, "-")
      ...
      cat $file | awk -v start_date="$start_date" -v end_date="$end_date" -v start_time="$start_time" -v end_time="$end_time" -v username="$username" -v ip_address="$ip_address" '
        BEGIN {
        ...
        # Match the date and time in the log file
        $1 ~ /-/ && $2 ~ /:/ {
          # Convert the log date to seconds since epoch
          split($1, ld, "-")
          log_epoch = mktime(ld[3] " " ld[2] " " ld[1] " 0 0 0")
          # Convert the log time to seconds
          split($2, lt, ":")
          log_sec = lt[1] * 3600 + lt[2] * 60 + lt[3]
          # Check if the log date and time are within the range
          if (log_epoch >= start_epoch && log_epoch <= end_epoch && log_sec >= start_sec && log_sec <= end_sec) {
...
check_sftp_logs() {
  for file in /var/log/secure*; do
    if file $file | grep -q gzip; then # Check if the file is in gzip format
      zcat -q $file | awk -v start_date="$start_date" -v end_date="$end_date" -v start_time="$start_time" -v end_time="$end_time" -v username="$username" -v ip_address="$ip_address" '
        BEGIN {
          split(start_date, sd, "-")
          start_epoch = mktime(sd[3] " " sd[2] " " sd[1] " 0 0 0")
...
check_ftp_logs() {
  for file in /var/log/xferlog*; do
    if file $file | grep -q gzip; then
      zcat -q $file | awk -v start_date="$start_date" -v end_date="$end_date" -v start_time="$start_time" -v end_time="$end_time" -v username="$username" -v ip_address="$ip_address" '
        BEGIN {
          split(start_date, sd, "-")
